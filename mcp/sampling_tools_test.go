...
func TestCreateMessageResult_ToolUseContent(t *testing.T) {
	// Test that CreateMessageResult can unmarshal tool_use content
	jsonData := `{
	...
	if !ok {
		t.Fatalf("Content expected *ToolUseContent, got %T", result.Content)
	}
...
func TestCreateMessageParams_WithTools(t *testing.T) {
	params := &CreateMessageParams{
		MaxTokens: 1000,
		Messages: []*SamplingMessage{
			{
				Role:    "user",
				Content: &TextContent{Text: "Calculate 1+1"},
			},
		},
		Tools: []*Tool{
			{
	...
	var params2 CreateMessageParams
...
func TestSamplingWithTools_Integration(t *testing.T) {
	...
	var receivedParams *CreateMessageParams

	// Client with tools capability
	...
	// Server sends CreateMessage with tools
	result, err := ss.CreateMessage(ctx, &CreateMessageParams{
		MaxTokens: 1000,
		Messages: []*SamplingMessage{
	...
	toolUse, ok := result.Content.(*ToolUseContent)
	if !ok {
		t.Fatalf("Content type = %T, want *ToolUseContent", result.Content)
	}
