...
func TestServerDefaultCacheable(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			defaults: &Cacheable{TTLMs: 60_000, CacheScope: "private"},
			want:     Cacheable{TTLMs: 60_000, CacheScope: "private"},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			server := NewServer(testImpl, &ServerOptions{DefaultCacheable: tc.defaults})
...
func TestSetDefaultCacheableValues(t *testing.T) {
	t.Run("nil defaults", func(t *testing.T) {
		c := Cacheable{TTLMs: 42}
		c.setDefaultCacheableValues(nil)
		if c.TTLMs != 42 || c.CacheScope != "public" {
			t.Fatalf("got %+v, want TTLMs preserved and CacheScope public", c)
		}
	})
	t.Run("explicit defaults", func(t *testing.T) {
		c := Cacheable{TTLMs: 1, CacheScope: "public"}
		c.setDefaultCacheableValues(&Cacheable{TTLMs: 60_000, CacheScope: "private"})
