...
// TestSubscriptionsListen_Streamable opens two concurrent subscriptions/listen
// streams on the same client session against a stateless HTTP server and
// verifies the full SEP-2575 contract end-to-end as observed by the client:
//
//   - both listens are acknowledged, each tagged with its own request ID;
//   - opt-in filtering: tool-list-changed reaches only the tools listen and
//     prompt-list-changed reaches only the prompts listen;
//   - the underlying SSE streams persist across multiple unrelated changes so
//     notifications keep arriving until the session is closed;
//   - subscription-ID tagging: every notification carries the originating
//     listen's request ID in `_meta`, matching the ack's tag for that listen;
//   - closing the client session ends all deliveries.
func TestSubscriptionsListen_Streamable(t *testing.T) {
	...
	cs, err := client.Connect(context.Background(), &StreamableClientTransport{Endpoint: httpServer.URL},
		&ClientSessionOptions{protocolVersion: protocolVersion20260630})
	if err != nil {
		t.Fatalf("client connect: %v", err)
	}

	startListen := func(notifs NotificationSubscriptions) {
		go func() {
			_ = cs.SubscriptionsListen(context.Background(), &SubscriptionsListenParams{Notifications: notifs})
		}()
	}
	...
	startListen(NotificationSubscriptions{ToolsListChanged: true})
	startListen(NotificationSubscriptions{PromptsListChanged: true})
	...
	// Trigger a tool change. The notification's subscription ID identifies
	// which of the two acks belongs to the tools listen; the other ack
	// therefore belongs to the prompts listen.
	...
	if e := waitFor("tool"); e.id != toolSubID {
		t.Errorf("second tool notif id = %s, want %s", e.id, toolSubID)
	}
	server.AddPrompt(&Prompt{Name: "p3"}, nil)
	if e := waitFor("prompt"); e.id != promptSubID {
		t.Errorf("second prompt notif id = %s, want %s", e.id, promptSubID)
	}
	expectNoEvent(notificationDelay * 5)
