...
authHandler, _ := auth.NewAuthorizationCodeHandler(&auth.AuthorizationCodeHandlerConfig{
	...
	AuthorizationCodeFetcher: func(ctx context.Context, input *auth.AuthorizationInput) (*auth.AuthorizationResult, error) {
		// Open the input.URL in a browser and return the resulting code and state.
