...
// The listeverything command lists all features of a stdio MCP server.
//
// Usage: listeverything <command> [<args>]
//
// For example:
//
//	listeverything go run github.com/modelcontextprotocol/go-sdk/examples/server/hello
//
// or
//
//	listeverything npx @modelcontextprotocol/server-everything
package main
...
func main() {
	...
	args := flag.Args()
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: listeverything <command> [<args>]")
		fmt.Fprintf(os.Stderr, "List all features for a stdio MCP server")
		fmt.Fprintln(os.Stderr)
		fmt.Fprintf(os.Stderr, "Example: listeverything npx @modelcontextprotocol/server-everything")
		os.Exit(2)
	}
